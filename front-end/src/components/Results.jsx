...
export const Results = ({ name, age, weight, timesFeeded, setReqType }) => {
  ...
  useEffect(() => {
    ...
    if (isNaN(ageVal) || isNaN(wt) || isNaN(timesFeededVal))
  ...
  return (
    <div className="w-full h-dvh text-xl text-center">
        ...
        <caption>
          {ageVal > 12 ? (
            <h1 className="mb-4 text-red-500">
              {" "}
              Babies above 1 year are not considered as infants{" "}
            </h1>
          ) : (
            <h1 className="font-bold mb-4">{`Results for baby ${name}`}</h1>
          )}
        </caption>
        <caption>
          <h1 className="mb-4 text-red-500">{error}</h1>
        </caption>
        <tbody>
          <tr>
            <th className="border-2 border-slate-950 p-4 bg-slate-400">
              Food Item
            </th>
            <th className="border-2 border-slate-950 p-2 bg-slate-400">
              From(quantity)
            </th>
            <th className="border-2 border-slate-950 p-2 bg-slate-400">
              To(quantity)
          ...
          <tr>
            ...
            <td className="border-2 border-slate-950 p-2">
              {fromValVegetables}
            </td>
      ...
      <button
        ...
        className="w-full sm:w-1/6 rounded-md p-1.5 mt-12 border-2 bg-sky-600 text-slate-100 border-gray-950 text-lg"
      >
